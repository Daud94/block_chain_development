...
class BlockchainController {
    ...
    validateChain(){
        this.app.get("/blocks/validate-chain", async (request, response) => {
                let error_log = await this.blockchain.validateChain()
                if (error_log.length > 0) {
                    return response.status(500).send("Invalid chain and invalid blocks");
                } else {
                    return response.status(200).send("Valid chains and blocks")
                }
        })
    }
