...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            await self.validateChain().then(function(error_log){
                if (error_log && error_log.length) {
                    reject("Errors validating the chain")
                } else {
            ...
            if (block.hash) {
                ...
                reject("Error: block adding failed");
            }
        });
    }
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (currentTime - message_time > 300) {
                reject('Time elapsed')
            }
            ...
            let new_block = new BlockClass.Block({owner: address, star: star})
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            let block = self.chain.filter(it => it.hash === hash)[0]
            if (block) {
                resolve(block)
            } else {
    ...
    getStarsByWalletAddress(address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(async function (block) {
                await block.getBData().then(function (value) {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            for (const block of self.chain) {
                const height = self.chain.indexOf(block);
